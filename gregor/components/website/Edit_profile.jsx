...
const Edit_profile = () => {
    const {
        ...
        setUpdatingStatus,
        openModal,
        edit_user_values: values,
        set_edit_user_values: setValues,
    } = useStateContext();



    const [user_photo_state, set_user_photo_state] = useState(null);
    const [formData, setFormData] = useState(null)
    const add_change_user_photo = async (e) => {

        set_user_photo_state(e.target.files[0]);
        const form = new FormData();
        form.append('file', e.target.files[0]);
        form.append("upload_preset", "myCloud")
        form.append("cloud_name", "dceqyrfhu");
        setFormData(form);
    ...
    useEffect(() => {
        if (cookieUser) {
            setValues({
                ...
                profilePhoto: cookieUser.profilePhoto,
            })
        }
    }, [cookieUser])
    // it's gonna show if user has updated smth.
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.values({ ...errors, ...errors2 }).every(e => !e)) {
            ...
            if (user_photo_state) {
                const url = await update_user_photo(formData);
                updateObj = { ...updateObj, profilePhoto: url }
            }
            handleUpdateUserAPI({ ...updateObj, _id: cookieUser.id, }, setUpdatingStatus, passwordStateRevert, "", "", set_user_photo_state);
        }
            ...
            {cookieUser &&
                <form onSubmit={handleSubmit} className='w-full lg:w-[450px] flex flex-col items-center gap-5 md:gap-6 pt-24 lg:pt-6 mb-6' >
                    ...
                    <div className='flex w-full flex-col justify-center items-center gap-4' >
                        <Image width="150" height="150" src={user_photo_state ? URL.createObjectURL(user_photo_state) : values.profilePhoto || unknown} alt="user" className='object-cover rounded-full w-[150px] h-[150px] border-4 border-stone-300' />

                        {user_photo_state || values.profilePhoto ?
                            <div className='flex w-full justify-center gap-4'>
                                <button
                                    type='button'
                                    onClick={() => openModal("delete_user_photo_modal")}
                                    className={` border-red-600 bg-red-600 hover:opacity-80
                                ...
                                border rounded-md px-[18px] py-[6px] text-white text-[12px] md:text-[14px] transition-all cursor-pointer`} >
                                    Change Photo
                                </label>
                                <input accept='image/*' type="file" onChange={add_change_user_photo} id="change-photo" className='hidden' />
                            </div>
                            :
                            <div>
                                <label
                            ...
                            border rounded-md px-[18px] py-[6px] text-white text-[12px] md:text-[14px] transition-all cursor-pointer`} >
                                ...
                                <input accept='image/*' onChange={add_change_user_photo} type="file" id="add-photo" className='hidden' />
                            </div>
                        }
